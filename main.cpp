...
bool isValidTagOpeningAndClosing(string open, string close) {
    ...
    {
        if (open[i] != close[i + 1]){return false;}
    }
    return true;
}
...
bool openFile(const char *fileName) {
    ...
    while (getline(cin, currentLine)) {
        for (char c: currentLine) {
            ...
            {
                cout << "Problem at Line : " << lineNumber << ", char : " << charNumber
                ...
                if (bracketStack.size() != 1 || bracketStack.top() != '<') //Bracket opened but not closed
                {
                    ...
                    tagOpened = false;
                    if (tag[0] == '/' &&
                        !tagStack.empty()) // if this is a closing tag, check that it is the tag for the last open tag
                    {
                        ...
                        {
                            cout << "Problem at Line : " << lineNumber << ", char : " << charNumber
                                 << " (Invalid closing tag)." << "\"" << tagStack.top() << "\"" << endl;
                            // return false;
                        }
                    }
    ...
    while (!tagStack.empty()) {
        cout << "Problem at Line : " << tagLines.top().first << ", char : " << tagLines.top().second << " (Tag \""
             << tagStack.top() << "\" not closed)." << endl;
        tagStack.pop();
        tagLines.pop();
    }
    return true;
